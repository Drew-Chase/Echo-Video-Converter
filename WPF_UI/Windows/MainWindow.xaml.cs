...
namespace ChaseLabs.Echo.Video_Converter.Windows
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private readonly double seconds = 2;
        private long currentTime = DateTime.Now.Ticks, neededTime = 0;

        public MainWindow()
        {
            InitializeComponent();
            onStartUp();
            //RedundantLauncherUpdateCheck();
        ...
        void RedundantLauncherUpdateCheck()
        {
        ...
        private async void ProcessBtn_Click(object sender, RoutedEventArgs args)
        {
            ...
            foreach (MediaFile value in file.Get(SortType.Size, SortOrder.Ascending))
            {
                ...
                {
                    log.Error($"Unknown Error Has Occurred while Processing {value}", e);
                }
            }
            ProcessBtn.IsEnabled = true;
        ...
        {
            if (encode_util == null || encode_util.process == null || HasAborted)
            {
                Close();
                return;
            }
